...
from Features.music import player
from Body.speak import Speak
from Normal.Newread import latestnews
import time
import random
import requests
import pyautogui
...
import numpy as np 
import random
from Features.close import Taskcomplier
from whatsppy import WhatsappSender
from Normal.Newread import latestnews
...
def EveryTask(Query):
    ...
    try:
        if "open" in ReturnData:
            ...
            return Value
        elif "ip address" in ReturnData:
            ip = requests.get('https://api.ipify.org').text
            Speak(f"Your ip address is {ip}")
            return Value
        
        elif "news" in ReturnData:
            latestnews()
            return Value
        
        elif "note" in ReturnData:
            notepad()
            return Value
          
        elif "switch the window" in ReturnData or "switch window" in ReturnData:
        ...
        elif "play music" in ReturnData:
         player.play_random_song()
         return Value
    


        elif "stop the music" in ReturnData:
         player.pause()
         return Value
         
        elif "where i am" in ReturnData or "current location" in ReturnData or "where am i" in ReturnData:
            try:
